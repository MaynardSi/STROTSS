...
class Solver:
    def __init__(self, content_image, content_path, style_path, long_side, scale):
        self.scale = scale #scale for saving only
        self.content_path = content_path #content path string needed for saving only

        self.style_path = style_path
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.phi = LossNetwork()
        ...
        self.style_image = load_path_for_pytorch(style_path, max_side = long_side, force_scale=True).unsqueeze(0).to(self.device)
        
    def style_transfer(self, pastiche_image, moment_weight, content_weight, lr=2e-3, num_iterations=250):
        ...
        for i in tqdm(range(num_iterations)):
            ...
            with torch.no_grad():
                self.content_features, content_features_raw = self.phi.features_subsample(i, self.content_image, image_type='content', n_samples = self.n_samples, random_seed=self.random_seed, detach=True)

                ### Extract style features ###
                self.style_features, style_features_raw = self.phi.features_subsample(i, self.style_image, image_type='style', n_samples=self.n_samples, random_seed=self.random_seed, detach=True)

            self.style_features = self.style_features.squeeze().transpose(0, 1)
            self.content_features = self.content_features.squeeze().transpose(0, 1)

            pastiche_features, pastiche_features_raw = self.phi.features_subsample(i, pastiche_image, image_type='pastiche', n_samples=self.n_samples, random_seed=self.random_seed, detach=False)
            
            pastiche_features = pastiche_features.squeeze().transpose(0, 1)

            loss_content = compute_content_loss(pastiche_features, self.content_features)

            loss_style = compute_style_loss(pastiche_features, self.style_features, moment_weight = moment_weight, content_weight=content_weight)
            ...
            #     # print("content feature extraction: ", end_cfe)
            #     # print("style feature extraction: ", end_sfe)
            #     # print("pastiche feature extraction: ", end_pfe)
            #     # print("content loss: ", end_lc)
            #     # print("style loss: ", end_ls)
            #     # print("backward: ", end_b)
        ...
        torch.save({
            'scale': self.scale,
            'content_features_sampled': self.content_features,
            'style_features_sampled': self.style_features,
            'random_seed' : self.random_seed,
            'lapl_pyramid' : lap_pyramid,
            'pastiche_image' : pastiche_image
            }, "model/{}_C[{}]_St[{}]_S{}.pth".format(time_string, (os.path.basename(self.content_path)).replace(".jpg",""), (os.path.basename(self.style_path)).replace(".jpg",""), self.scale))
